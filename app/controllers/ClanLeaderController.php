...
class ClanLeaderController {
    ...
    /**
     * DEBUG: Verificar todas las tareas asignadas del clan
     */
    public function debugTeamTasks() {
        header('Content-Type: application/json');
        ...
        try {
            ...
            // 1. Verificar todas las tareas del proyecto del clan
            $stmt1 = $this->db->prepare("
                SELECT 
                    ...
                    p.project_name,
                    p.is_personal,
                    p.clan_id as project_clan_id,
                    u.full_name as assigned_user_name,
                    cm.clan_id as user_clan_id
                FROM Tasks t
                JOIN Projects p ON t.project_id = p.project_id
                LEFT JOIN Users u ON t.assigned_to_user_id = u.user_id
                ...
                LIMIT 20
            ");
            $stmt1->execute([$clanId]);
            $allClanTasks = $stmt1->fetchAll(PDO::FETCH_ASSOC);
            
            // 2. Verificar miembros del clan
            $stmt2 = $this->db->prepare("
                SELECT u.user_id, u.full_name, cm.clan_id
                FROM Users u
                JOIN Clan_Members cm ON u.user_id = cm.user_id
                WHERE cm.clan_id = ?
            ");
            $stmt2->execute([$clanId]);
            $clanMembers = $stmt2->fetchAll(PDO::FETCH_ASSOC);
            ...
            Utils::jsonResponse([
                ...
                'debug_info' => [
                    ...
                    'clan_id' => $clanId,
                    'all_clan_tasks' => $allClanTasks,
                    'clan_members' => $clanMembers,
                    'total_clan_tasks' => count($allClanTasks),
                    'total_clan_members' => count($clanMembers)
                ]
            ]);
